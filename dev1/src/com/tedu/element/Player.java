 ...
 * 玩家角色类
 */
public class Player extends GardenObj {
	...
	private int ttl = 72; 		//泡泡爆炸时间
	// --- 新增：记录当前角色皮肤的键 ---
	private String skinKey;     // 例如 "player1" / "player2" / "player3"
	...
	public GardenObj createElement(String str) {
		...
		ImageIcon icon = GardenResourceLoader.imgMap.get(split[2]);
		this.setIcon(icon);
		this.skinKey = split[2];
...
//						this.getW(), this.getH(), null);
		...
		if (dieAnimateStartTime == -1) {
			...
			g.drawImage(GardenResourceLoader.imgMap.get("SurroundedByBubbles").getImage(),
					// Player在面板中显示的区域。一个Player占据一个格子的大小（48*48）
					this.getX(), this.getY(),  // 左上角坐标
...
//		System.out.println("correctedX=" + correctedX + ";correctedY=" + correctedY);
		...
		// 最后需要将面板大小精准控制为地图大小
		// 右
		if (dirFlag[2] && this.getX() < DreamGardenFrame.jp3.getWidth() - 48) {
			this.setX(this.getX() + moveSpeed);
		}
		
		// 下
		if (dirFlag[3] && this.getY() < DreamGardenFrame.jp3.getHeight() - 48 - 4) {
			// 减4是为了修正误差，误差原因未知
			this.setY(this.getY() + moveSpeed);
		}
	}
	...
	protected void add() { 
		...
		GardenObj element=new MagicCircle().createElement(this.toString());
		GardenResourceLoader.playMusic("layBubble");
		
		//装入到集合中
		SceneElementManager.getManager().addElement(element, GardenElement.PAOPAO);
		...
		if (!isPressed) {
			if (curDir == 0 || curDir == 2) {
				...
				if (this.getX() % 24 != 0) {
					dirFlag[curDir] = true;
					if (curDir == 0) {
						correctedX = this.getX()/24*24;
					} else if (curDir == 2) {
						correctedX = (this.getX()/24+1)*24;
					}
				} 
			}
			...
			if (curDir == 1 || curDir == 3) {
				...
				if (this.getY() % 24 != 0) {
					dirFlag[curDir] = true;
					if (curDir == 1) {
						correctedY = this.getY()/24*24;
					} else if (curDir == 3) {
						correctedY = (this.getY()/24+1)*24;
					}
				}
			}
		}
...
//		System.out.println(isCollided);
		 ...
		 */
		if (obj instanceof MagicCircle) {
//			System.out.println(isCollided);
			MagicCircle paopao = (MagicCircle) obj;
			if (isCollided) {
