...
public class MagicCircle extends GardenObj {
	...
	@Override
	public GardenObj createElement(String str) {//定义字符串的规则
		...
		for(String str1 : split) {
			...
			case "ttl": ttl=Integer.parseInt(split2[1]);break;
			}
		}
		ImageIcon icon = GardenResourceLoader.imgMap.get("paopao");
		this.setIcon(icon);
		this.setW(48);
		this.setH(48);
		return this;
	...
	public void reloadBubble(GardenElement ge){
		List<GardenObj> list2 = SceneElementManager.getManager().getElementsByKey(ge);
	...
	public void die(long gameTime) { //死亡也是一个对象
		if(!isLive()) {	
			GardenObj element=new CircleBurst().createElement(this.toString());
			//装入到集合中
			SceneElementManager.getManager().addElement(element, GardenElement.EXPLODE);
			GardenResourceLoader.playMusic("bomb");
			//改变炸弹玩家已经放在炸弹数bubbleNum,显示爆炸效果，加入ExplodeBubble
			reloadBubble(GardenElement.PLAYER);
			reloadBubble(GardenElement.ENEMY);
