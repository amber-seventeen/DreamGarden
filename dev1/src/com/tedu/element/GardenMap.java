 ...
 */
public class GardenMap extends GardenObj {
	...
	public GardenObj createElement(String str) {
		...
		if(split[0].equals("tree") || split[0].equals("yellowhouse") || split[0].equals("bluehouse")) {
			...
			icon = GardenResourceLoader.imgMap.get("map2");
		}
		else {
			...
			icon = GardenResourceLoader.imgMap.get("map1");
		}		
	...
	public void die(long gametime) {
		if(!this.isLive())
		{
			...
			if(r<=50&&r>0)
			{
				GardenObj obj=new GardenTool().createElement(str);
				SceneElementManager.getManager().addElement(obj, GardenElement.TOOL);
				return;
			}
			else {
				return ;
